...
module.exports = class {
  ...
  async create(userEntity) {
    const { firstName, lastName, type, email, password } = userEntity;
    const seqUser = await this.model.create({ firstName, lastName, type, email, password });
    await seqUser.save();

    return new User(seqUser.id, seqUser.firstName, seqUser.lastName, seqUser.type, seqUser.email, seqUser.password);
  }
  ...
  async update(userEntity) {
    ...
    const { firstName, lastName, type, email, password } = userEntity;
    await seqUser.update({ firstName, lastName, type, email, password });
